...
namespace StudentEnd
{
    public static class Startup
    {
        public static App Init(Action<HostBuilderContext, IServiceCollection> nativeConfigureServices)
        {
            ...
            ExtractSaveResource("StudentEnd.appsettings.json", systemDir);
            var fullConfig = Path.Combine(systemDir, "StudentEnd.appsettings.json");
        ...
        static void ConfigureServices(HostBuilderContext context, IServiceCollection services)
        {
            ...
            services.AddSingleton(
                new LoginDatabaseService(Path.Combine(FileSystem.AppDataDirectory, "StudentEnd.db3")));

            services.AddSingleton(new FaceRecognitionService(context.Configuration["BaiduAI:AppId"],
                context.Configuration["BaiduAI:ApiKey"],
                context.Configuration["BaiduAI:SecretKey"]));
